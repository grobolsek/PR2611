"""Core data-loading and transformation functions for the formatting pipeline.

Provides the building blocks consumed by :mod:`formatting.formatter` to clean
and normalise raw crime-statistics CSV files:

- Reading and discovering raw CSV files.
- Column type conversions (booleans, ordered categoricals).
- Structural transformations (melt, split, repair).
- Crime-cluster mapping and value masking.

Exploratory / validation helpers (unique value inspection, format checking)
live in :mod:`formatting.analysis`.
"""
...
CRIME_CLUSTERS: dict[str, list[str]] = {
    "Nasilje nad osebami": [
        "UBOJ",
        "UMOR",
        "UGRABITEV",
...
# Columns whose names changed across yearly files → canonical name
COLUMN_ALIASES: dict[str, str] = {
    "DatumStoritve": "MesecStoritve",
    "OrganizacijskaEnotaStoritve": "PUStoritveKD",
    "UpravnaEnota": "UpravnaEnotaStoritve",
    "VrstaZakDokumenta": "VrstaZakljucnegaDokumenta",
}


def normalize_columns(data: pd.DataFrame, source_name: str = "") -> pd.DataFrame:
    """Rename aliased columns to canonical names and add any missing required columns as NaN.

    Args:
        data: Raw DataFrame as loaded from CSV.
        source_name: File name used in log messages.

    Returns:
        DataFrame with canonical column names; missing required columns added as NaN.
    """
    data = data.rename(columns=COLUMN_ALIASES)

    for col in REQUIRED_COLUMNS:
        if col not in data.columns:
            logger.warning("Column %r missing in %s — adding as NaN.", col, source_name or "input")
            data[col] = None
...
def get_data(input_file: Path) -> pd.DataFrame:
    """Read a raw crime-statistics CSV file and return a DataFrame.

    Args:
        input_file: Path to the CSV file to read.

    Returns:
        Parsed :class:`~pandas.DataFrame`, or an empty DataFrame if reading
        fails.
    """
    try:
        return pd.read_csv(
        ...
        logger.warning("Error processing %s: %s", input_file.name, e)
        return pd.DataFrame()
...
def to_bool(
    ...
    count as ``'DA'``, only those exact values (after stripping) produce
    ``'DA'``; everything else (including NaN and whitespace-only strings)
    produces ``'NE'``.
    ...
    Returns:
        DataFrame with the specified columns replaced by ``'DA'``/``'NE'``
        values.
    """
    data = data.copy()
...
def melt_numbered_columns(
    ...
    Args:
        data: Input DataFrame.
        prefixes: Column-name prefixes to melt (e.g.
            ``['KriminalisticnaOznacba', 'UporabljenoSredstvo']``).
...
def repair_by_group(
    ...
    - Invalid rows are filled with the valid value from the group if all valid
      rows agree on a single value.
    - Groups with no valid value at all are dropped entirely.
    ...
    logger.info("repair_by_group [%s]: fixed %d, removed %d (%s success rate)", col, fixed_rows, dropped, pct)
    return data[keep_mask].reset_index(drop=True)


def add_crime_cluster(data: pd.DataFrame, crime_col: str) -> pd.DataFrame:
    """Add a ``KD_SKUPINA`` column mapping each crime description to its cluster.

    Uses :data:`CRIME_CLUSTERS` to build the mapping. Crimes not found in any
    cluster are labelled ``'Neznano'``.
