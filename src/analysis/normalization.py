...
Provides function-based utilities that return normalized tables depending on
...
Example usage::
    from formatting.normalization import (
        crimes_per_population,
        normalize_by_hour,
        crime_type_distribution,
    )
    df = pd.read_csv('data/formatted/kd2024.csv')
    rates = crimes_per_population(df, year=2024)
    by_hour = normalize_by_hour(df)
    dist = crime_type_distribution(df)
"""
...
def _extract_start_hour(ura: object) -> int | None:
...
def normalize_by_hour(crimes: pd.DataFrame, city_col: str = "PUStoritveKD", hour_col: str = "UraStoritve") -> pd.DataFrame:
    """Return hourly percentage distribution of crimes per city.

    Result columns: `location`, `hour`, `count`, `pct_of_city` (0-100).
    """
    df = crimes.copy()
    df[city_col] = df[city_col].apply(_normalize_location)
    df["_start_hour"] = df[hour_col].apply(_extract_start_hour)
...
def crime_type_distribution(crimes: pd.DataFrame, city_col: str = "PUStoritveKD", crime_col: str = "KD_SKUPINA") -> pd.DataFrame:
