...
def get_crime_weather_data(
    ...
    crime_counts = filtered_crimes.groupby([month_col, city_col], dropna=False).size().reset_index(name="stevilka_zlocinov")
...
def run_weather_correlation_analysis(year: int, base_directory: Path) -> None:
    ...
    if not formatted_file.exists():
        logger.error(f"Cleaned crime dataset for year {year} does not exist: {formatted_file}")
        return
    if not weather_file.exists():
        logger.error(f"Weather dataset for year {year} does not exist: {weather_file}")
        return

    logger.info(f"Loading datasets and initiating correlation analysis for year {year}...")
